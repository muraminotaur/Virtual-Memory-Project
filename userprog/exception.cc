...
ExceptionHandler(ExceptionType which)
{
	...
	case PageFaultException : {
		bool pageTableAllocated = currentThread->space->fillPageTable();
		ASSERT(pageTableAllocated);
		int temp = (machine->ReadRegister(BadVAddrReg) / PageSize)  * PageSize;
    	printf("Temp value: %d \n", temp);
		currentThread->space->readPage();
		stats->numPageFaults++;
		break;
	}
