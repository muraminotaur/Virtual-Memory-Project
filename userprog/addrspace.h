...
class AddrSpace {
  public:
    ...
    bool Valid();     // check for allocation failure
    // End Changes by Jacob Slagle

    //start code changes by Wyatt Woodall
    bool fillPageTable();
    unsigned int getCurrentPhysPage();
    void readPage();
  ...
  private:
    ...
    unsigned int dynamicPages; //Number of pages that we've allocated through page faults
    OpenFile *sourceFile;
